...
const selectTranslation = document.getElementById('translation');


selectTranslation.addEventListener('change', function (e) {
    setCurrentLanguage(e.target.value);
});


function tr(messageID) {
    const trRegex = /\{\{tr\s(.+?)\}\}/g;
    const elements = document.querySelectorAll('[data-trValue]');
    for (const element of elements) {
        const text = element.textContent;
        const matches = text.match(trRegex);
        for (const match of matches) {
            const [, trMatch] = match.match(/\{\{tr\s(.+?)\}\}/);
            if (trMatch === messageID) {
                const translation = userMessages[currentLanguage][messageID];
                const newText = text.replace(match, translation);
...
const trDoc = (function () {
    ...
    return function trDoc(ancestorNode) {
        const elements = ancestorNode.querySelectorAll('[data-trValue]');
        for (const element of elements) {
            const text = element.getAttribute('data-trValue');
            const matches = text.match(trDocRe);
            for (const match of matches) {
                const [, trMatch] = match.match(/\{\{tr\s(.+?)\}\}/);
                try {
                    const translation = userMessages[currentLanguage][trMatch];
                    const newText = text.replace(match, translation);
                    element.textContent = newText;
                } catch (error) {
